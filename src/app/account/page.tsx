...
export default function AccountPage() {
      ...
      <main className="min-h-screen bg-[radial-gradient(circle_at_top,#dbeafe_0%,#eff6ff_38%,#f8fafc_72%,#ffffff_100%)] px-4 py-8 text-slate-950">
        <div className="mx-auto max-w-3xl rounded-[28px] border border-slate-200/80 bg-white/95 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
          ...
          <p className="mt-3 text-sm text-slate-600">
            This is the first account foundation for the private player area. Sign in with your player account or create
            one linked to a player or enrollment request.
          </p>
            ...
            {mode === "register" ? (
              <>
                <input
                  ...
                  placeholder="Player documentId (optional)"
                  className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none"
                />
                <input
                  value={enrollmentRequestId}
                  onChange={(e) => setEnrollmentRequestId(e.target.value)}
                  placeholder="Enrollment request documentId (optional)"
                  className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 outline-none"
                />
              </>
            ) : null}
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#dbeafe_0%,#eff6ff_38%,#f8fafc_72%,#ffffff_100%)] px-4 py-8 text-slate-950">
      <div className="mx-auto max-w-4xl rounded-[28px] border border-slate-200/80 bg-white/95 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
        ...
        {!account.emailVerifiedAt ? (
          <div className="mt-6 rounded-2xl bg-amber-50 px-4 py-4 text-sm text-amber-900">
            ...
            <p className="mt-2">
              Verify your email to secure your account recovery and future account actions.
            </p>
            <button
              type="button"
              onClick={async () => {
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-slate-50 px-4 py-3">
            ...
            <div className="mt-2 text-lg font-semibold text-slate-950">{statusLabel(account.status)}</div>
          </div>
          <div className="rounded-2xl bg-slate-50 px-4 py-3">
            <div className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Player</div>
            <div className="mt-2 text-lg font-semibold text-slate-950">
              {account.player?.documentId || "Not linked yet"}
            </div>
          </div>
          <div className="rounded-2xl bg-slate-50 px-4 py-3">
            <div className="text-[11px] uppercase tracking-[0.18em] text-slate-500">Enrollment</div>
            <div className="mt-2 text-lg font-semibold text-slate-950">
              {account.enrollmentRequest?.status || "None"}
            </div>
          </div>
        </div>

        {account.enrollmentRequest ? (
          <div className="mt-6 rounded-2xl bg-amber-50 px-4 py-3 text-sm text-amber-800">
            Enrollment request: <span className="font-semibold">{account.enrollmentRequest.documentId}</span>
            {account.enrollmentRequest.accountCompletionStatus ? (
              <> · Completion: <span className="font-semibold">{account.enrollmentRequest.accountCompletionStatus}</span></>
            ) : null}
          </div>
        ) : null}

        <div className="mt-8 rounded-2xl border border-dashed border-slate-300 px-4 py-6 text-sm text-slate-500">
          Phase 1 foundation is active. Next steps: tournaments, friendly matches, devices, and profile completion flow.
        </div>
      </div>
    </main>
  );
