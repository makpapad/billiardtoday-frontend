...
export function LiveClubView({ club, embedded = false }: Props) {
  ...
  if (embedded) {
    return (
      <main className="min-h-screen overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(34,211,238,0.16),_transparent_34%),linear-gradient(160deg,#020617_0%,#081226_42%,#0f172a_100%)] px-3 py-3 sm:px-4 sm:py-4">
        <div className="mx-auto flex max-w-7xl flex-col gap-3">
          <section className="relative overflow-hidden rounded-[28px] border border-cyan-200/20 bg-slate-950/55 px-4 py-4 shadow-[0_18px_80px_rgba(2,6,23,0.45)] backdrop-blur-xl">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.18),_transparent_28%),radial-gradient(circle_at_bottom_right,_rgba(59,130,246,0.18),_transparent_34%)]" />
            <div className="relative flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div className="min-w-0">
                <div className="text-[10px] font-semibold uppercase tracking-[0.42em] text-cyan-200/70">
                  Live Embed
                </div>
                ...
                <div className="flex flex-wrap gap-2 lg:max-w-[48%] lg:justify-end">
                  ...
                  {tournamentOptions.map((option) => (
                    <button
                      key={option}
                      type="button"
                      onClick={() => setSelectedTournament(option)}
                      className={`max-w-full truncate rounded-full px-3 py-1.5 text-xs font-semibold transition ${
          ...
          {filteredItems.length === 0 ? (
            <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-slate-950/45 px-6 py-10 text-center shadow-[0_16px_70px_rgba(2,6,23,0.4)] backdrop-blur-xl">
              <div className="mx-auto flex max-w-xl flex-col items-center gap-3">
                <div className="flex h-14 w-14 items-center justify-center rounded-full border border-cyan-200/20 bg-cyan-300/10 text-cyan-100">
                  <span className="text-xl font-black">00</span>
                </div>
                <h2 className="text-xl font-semibold text-white">No active live tables</h2>
                <p className="max-w-md text-sm leading-6 text-white/60">
                  The embed feed is connected. This club currently has no pending or running scoreboard sessions.
                </p>
              </div>
            </section>
          ) : (
            <section className="grid gap-3 xl:grid-cols-2">
              {filteredItems.map((item) => (
                <EmbeddedLiveCard key={item.screenId || item.sessionId} item={item} />
              ))}
            </section>
          )}
        </div>
...
function formatEmbedUpdatedAt(value?: string) {
...
function formatEmbedHHMM(value?: number | null) {
...
function EmbeddedLiveCard({ item }: { item: LiveScoreItem }) {
  ...
  const duration = state.gameDurationSeconds ?? null;
  ...
  const buildStats = (
    ...
    const targetLabel =
      target && target > 0 && typeof points === "number"
        ? `${Math.max(0, Math.min(100, Math.round((points / target) * 100)))}%`
        : "--";

    return [
      { label: "AVG", value: avg || "--" },
      { label: "HR", value: typeof bestRun === "number" ? String(bestRun) : "--" },
      { label: "RUN", value: typeof liveRun === "number" ? String(liveRun) : "--" },
      { label: "TARGET", value: targetLabel },
    ];
  };

  return (
    <article className="relative overflow-hidden rounded-[30px] border border-cyan-200/15 bg-[linear-gradient(145deg,rgba(8,15,31,0.95)_0%,rgba(12,26,57,0.94)_55%,rgba(8,15,31,0.98)_100%)] p-4 shadow-[0_18px_80px_rgba(2,6,23,0.45)]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.14),_transparent_30%),radial-gradient(circle_at_bottom_right,_rgba(250,204,21,0.12),_transparent_34%)]" />
      <div className="relative flex flex-col gap-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold uppercase tracking-[0.24em] text-cyan-200/70">
            ...
            <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-white/55">
              ...
              <span>Updated {formatEmbedUpdatedAt(item.updatedAt)}</span>
            </div>
          </div>
          <div
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_auto_1fr] lg:items-stretch">
          <EmbedPlayerPanel
            side="left"
            active={playerAActive}
            name={state.playerAName || "Player A"}
            country={state.playerACountry ?? null}
            photoUrl={state.playerAPhotoUrl ?? null}
            score={state.scoreA ?? 0}
            stats={buildStats(
              state.avgFormattedA ?? null,
              state.bestRunA ?? null,
              state.liveRunA ?? null,
              state.targetPointsA ?? null,
              state.scoreA ?? 0,
            )}
          />

          <div className="flex min-w-[120px] flex-row gap-2 lg:flex-col">
            <EmbedCenterBadge label="INN" value={String(innings || 0)} />
            <EmbedCenterBadge
              label="TIME"
              value={duration && Number.isFinite(duration) ? formatEmbedHHMM(duration) : "--:--"}
            />
          </div>

          <EmbedPlayerPanel
            side="right"
            active={playerBActive}
            name={state.playerBName || "Player B"}
            country={state.playerBCountry ?? null}
            photoUrl={state.playerBPhotoUrl ?? null}
            score={state.scoreB ?? 0}
            stats={buildStats(
              state.avgFormattedB ?? null,
              state.bestRunB ?? null,
              state.liveRunB ?? null,
              state.targetPointsB ?? null,
              state.scoreB ?? 0,
            )}
          />
        </div>
...
function EmbedPlayerPanel({
  ...
  const accent =
    side === "left"
      ? "from-cyan-300/22 to-white/6"
      : "from-amber-300/22 to-white/6";
  const border = active
    ? side === "left"
      ? "border-cyan-300/40"
      : "border-amber-300/45"
  ...
  return (
    <div className={`rounded-[24px] border bg-gradient-to-br ${accent} ${border} p-4`}>
      <div className="flex items-center gap-3">
        <EmbedAvatar name={name} photoUrl={photoUrl} />
        <div className="min-w-0 flex-1">
          <div className="truncate text-lg font-semibold text-white">{name}</div>
          <div className="truncate text-sm text-white/50">{country || "Unknown country"}</div>
        </div>
      </div>
      <div className="mt-4 flex items-end justify-between gap-4">
        <div className="text-6xl font-black leading-none tracking-tight text-white">{score}</div>
        {active ? (
          <div className="rounded-full border border-white/15 bg-white/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-white/75">
            At table
          </div>
        ) : null}
      </div>
      <div className="mt-4 grid grid-cols-4 gap-2">
        {stats.map((stat) => (
          <div key={`${side}-${stat.label}`} className="rounded-2xl border border-white/10 bg-black/20 px-2 py-2 text-center">
            <div className="text-[9px] font-semibold uppercase tracking-[0.18em] text-white/45">{stat.label}</div>
...
function EmbedAvatar({ name, photoUrl }: { name: string; photoUrl?: string | null }) {
  ...
  return (
    <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-white/15 bg-white/10 text-sm font-semibold text-white">
      {initials}
    </div>
  );
}

function EmbedCenterBadge({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center rounded-[22px] border border-white/10 bg-black/25 px-4 py-3 text-center">
