...
export function LiveClubView({ club, embedded = false }: Props) {
  ...
  const videoDrawerSessions = React.useMemo<LiveVideoDrawerSession[]>(() => {
    return filteredItems.reduce<LiveVideoDrawerSession[]>((acc, item) => {
      ...
      acc.push({
        ...
        playerACountry: item.state?.playerACountry ?? null,
        playerBCountry: item.state?.playerBCountry ?? null,
        scoreA: item.state?.scoreA ?? null,
        scoreB: item.state?.scoreB ?? null,
        runA: item.state?.liveRunA ?? item.state?.runA ?? null,
        runB: item.state?.liveRunB ?? item.state?.runB ?? null,
        avgFormattedA: item.state?.avgFormattedA ?? null,
        avgFormattedB: item.state?.avgFormattedB ?? null,
        accPercentA: item.state?.accPercentA ?? null,
        accPercentB: item.state?.accPercentB ?? null,
        bestRunA: item.state?.bestRunA ?? null,
  ...
  const mobileVideoDrawerSessionId =
    videoDrawerSessionId ?? videoDrawerSessions[0]?.sessionId ?? null;
  const mobileVideoDrawerVideoId =
    videoDrawerVideoId ??
    videoDrawerSessions.find((session) => session.sessionId === mobileVideoDrawerSessionId)?.liveVideos[0]
      ?.videoId ??
    null;
