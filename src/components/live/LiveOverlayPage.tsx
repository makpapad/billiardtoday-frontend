...
export type LiveOverlaySearchParams = Record<string, string | string[] | undefined>;

type LiveOverlayPageProps = {
  params: LiveOverlaySearchParams | undefined;
  title: string;
  description: string;
...
function buildOverlayUrl(params: LiveOverlaySearchParams | undefined) {
      ...
      <section className="mx-auto flex min-h-screen w-full max-w-[1720px] flex-col px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-4 grid gap-3 text-sm text-slate-300 lg:grid-cols-3">
          <div className="border border-white/10 bg-white/[0.03] p-3">
            <div className="font-semibold text-white">BTDroitCamera/MediaMTX</div>
            <code className="mt-2 block break-all text-xs text-slate-400">
              {examplesBasePath}?provider=mediamtx&amp;overlay=Ceb1
            </code>
          </div>
          <div className="border border-white/10 bg-white/[0.03] p-3">
            <div className="font-semibold text-white">YouTube</div>
            <code className="mt-2 block break-all text-xs text-slate-400">
              {examplesBasePath}?provider=youtube&amp;stream=VIDEO_ID&amp;overlay=Ceb1
            </code>
          </div>
          <div className="border border-white/10 bg-white/[0.03] p-3">
            <div className="font-semibold text-white">HLS/video</div>
            <code className="mt-2 block break-all text-xs text-slate-400">
              {examplesBasePath}?provider=video&amp;stream=https://example.com/live.mp4&amp;overlay=Ceb1
            </code>
          </div>
        </div>
      </section>
