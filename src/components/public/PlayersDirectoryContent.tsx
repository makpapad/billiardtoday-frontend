...
export function PlayersDirectoryContent({ players, totalPlayers }: Props) {
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1320px] flex-col gap-10 px-4 py-8 sm:px-6">
      <PresentationHero
        eyebrow="Billiard Players"
        title="Billiard player profiles, results, and performance history"
        description="Search the public Billiard Today player directory for carom and 3-cushion billiard players, tournament records, countries, clubs, averages, high runs, rankings, and head-to-head match history."
        actions={[
          { label: "Explore tournaments", href: "/tournaments" },
          { label: "Browse clubs", href: "/clubs", variant: "secondary" },
        ]}
        meta={[
          `${availablePlayers.toLocaleString("en-US")} ${availablePlayers === 1 ? "player profile" : "player profiles"} indexed`,
      ...
      <section>
        <SectionHeading
          ...
          title="Find billiard players by name, country, city, or club"
          description="Start typing to search the full player database. Each result opens a public profile with tournament history, match records, averages, high runs, and performance trends where data is available."
        />
        <div className="mb-6">
