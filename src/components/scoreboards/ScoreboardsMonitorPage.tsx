...
function PlayerRow({
  ...
  return (
    <div
      className={
        active
          ? "flex items-center justify-between rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-3"
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2">
          <span
            className={
              active
                ? "h-2.5 w-2.5 rounded-full bg-cyan-500 shadow-[0_0_14px_rgba(6,182,212,0.55)]"
        ...
        <div className="mt-1 text-xs text-slate-500">
          {typeof innings === "number" && Number.isFinite(innings)
            ? `Innings ${innings}`
            : "Waiting innings"}
        </div>
      </div>
      ...
      <div className="mt-5 rounded-[28px] bg-slate-950 p-4 text-white">
        <div className="flex items-center justify-between gap-3 border-b border-white/10 pb-3">
          ...
          <div className="text-xs text-slate-300">
            {lastUpdate
              ? `WS ${formatRelativeTime(Date.parse(lastUpdate))}`
              : "Waiting data"}
          </div>
        </div>
