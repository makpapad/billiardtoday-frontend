...
export async function sendContactEmail(input: ContactFormPayload) {
  ...
  const confirmationHtml = [
    ...
    `<td align="right" style="font-size:13px"><a href="${absoluteUrl("/tournaments")}" style="display:inline-block;padding:10px 16px;border:1px solid rgba(255,255,255,0.18);border-radius:999px;color:#ffffff;text-decoration:none;font-weight:600">Tournaments</a></td>`,
    "</tr>",
    "</table>",
    ...
    "</div>",
    "<div style=\"padding:24px;background:#05080d;text-align:center\">",
    `<a href="${absoluteUrl("/")}" style="display:inline-block;text-decoration:none"><img src="${absoluteUrl("/logo-billiardtoday.png")}" alt="Billiard Today" width="180" style="display:block;width:100%;max-width:180px;height:auto;margin:0 auto;border:0" /></a>`,
    "</div>",
    "</div>",
    "</div>",
  ].join("");
