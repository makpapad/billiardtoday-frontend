 ...
 * @return array
 */
function billiardtoday_light_body_classes($classes) {
    ...
    $classes[] = 'billiardtoday-light';

    return $classes;
}
add_filter('body_class', 'billiardtoday_light_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function billiardtoday_light_pingback_header() {
    if (is_singular() && pings_open()) {
        printf('<link rel="pingback" href="%s">', esc_url(get_bloginfo('pingback_url')));
    }
}
add_action('wp_head', 'billiardtoday_light_pingback_header');

/**
 * Change the excerpt more string.
 */
function billiardtoday_light_excerpt_more($more) {
    return sprintf(
...
add_filter('excerpt_more', 'billiardtoday_light_excerpt_more');

/**
 * Custom comment callback.
 */
function billiardtoday_light_comment($comment, $args, $depth) {
    if ('div' === $args['style']) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 */
function billiardtoday_light_customizer_styles() {
...
add_action('wp_head', 'billiardtoday_light_customizer_styles');

/**
 * Add theme support for selective refresh for Customizer
 */
function billiardtoday_light_customize_partial_blogname() {
    bloginfo('name');
}
add_action('customize_partial_blogname', 'billiardtoday_light_customize_partial_blogname');

function billiardtoday_light_customize_partial_blogdescription() {
    bloginfo('description');
}
add_action('customize_partial_blogdescription', 'billiardtoday_light_customize_partial_blogdescription');

/**
 * Add WooCommerce support if WooCommerce is active
 */
if (class_exists('WooCommerce')) {
    function billiardtoday_light_woocommerce_setup() {
        add_theme_support('woocommerce');
        add_theme_support('wc-product-gallery-zoom');
        add_theme_support('wc-product-gallery-lightbox');
        add_theme_support('wc-product-gallery-slider');
    }
    add_action('after_setup_theme', 'billiardtoday_light_woocommerce_setup');
}

/**
 * Add custom page templates
 */
function billiardtoday_light_add_page_templates($templates) {
...
add_filter('theme_page_templates', 'billiardtoday_light_add_page_templates');

/**
 * Load custom page templates
 */
function billiardtoday_light_load_page_template($template) {
    global $post;
...
add_filter('template_include', 'billiardtoday_light_load_page_template');

/**
 * Add custom post types
 */
function billiardtoday_light_custom_post_types() {
...
add_action('init', 'billiardtoday_light_custom_post_types');

/**
 * Add custom taxonomies
 */
function billiardtoday_light_custom_taxonomies() {
...
add_action('init', 'billiardtoday_light_custom_taxonomies');

/**
 * Add meta boxes
 */
function billiardtoday_light_add_meta_boxes() {
    add_meta_box(
        'tournament_details',
        __('Tournament Details', 'billiardtoday'),
        'billiardtoday_light_tournament_details_callback',
        'tournaments',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'billiardtoday_light_add_meta_boxes');

function billiardtoday_light_tournament_details_callback($post) {
    wp_nonce_field('billiardtoday_light_save_tournament_details', 'tournament_details_nonce');
...
function billiardtoday_light_save_tournament_details($post_id) {
    if (!isset($_POST['tournament_details_nonce']) || !wp_verify_nonce($_POST['tournament_details_nonce'], 'billiardtoday_light_save_tournament_details')) {
        return;
    }
...
add_action('save_post', 'billiardtoday_light_save_tournament_details');
?>
