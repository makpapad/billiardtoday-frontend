<?php
/**
 * BilliardToday New Theme Functions
 */
...
// Theme Setup
function billiardtoday_new_setup() {
    ...
    add_theme_support('custom-header', array(
        ...
        'flex-height'            => true,
        'flex-width'             => true,
        'wp-head-callback'       => 'billiardtoday_new_header_style',
    ));
}
add_action('after_setup_theme', 'billiardtoday_new_setup');

// Custom header style
function billiardtoday_new_header_style() {
...
function billiardtoday_new_scripts() {
    // Theme stylesheet
    wp_enqueue_style('billiardtoday-new-style', get_stylesheet_uri(), array(), '1.0.0');
    
    // Google Fonts
    wp_enqueue_style('billiardtoday-new-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600&display=swap', array(), null);
    
    // Theme JavaScript
    wp_enqueue_script('billiardtoday-new-script', get_template_directory_uri() . '/js/theme.js', array('jquery'), '1.0.0', true);
    
    // Localize script
    wp_localize_script('billiardtoday-new-script', 'billiardtoday_new', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce'    => wp_create_nonce('billiardtoday_new_nonce'),
    ));
}
add_action('wp_enqueue_scripts', 'billiardtoday_new_scripts');

// Register widget areas
function billiardtoday_new_widgets_init() {
...
add_action('widgets_init', 'billiardtoday_new_widgets_init');
...
function billiardtoday_new_switcher() {
    if (isset($_POST['theme']) && isset($_POST['nonce'])) {
        if (wp_verify_nonce($_POST['nonce'], 'billiardtoday_new_nonce')) {
...
add_action('admin_post_switch_theme', 'billiardtoday_new_switcher');

// AJAX Theme Switcher
function billiardtoday_new_ajax_switch_theme() {
    check_ajax_referer('billiardtoday_new_nonce', 'nonce');
...
add_action('wp_ajax_switch_theme', 'billiardtoday_new_ajax_switch_theme');

// Add theme switcher to admin bar
function billiardtoday_new_admin_bar_switcher($wp_admin_bar) {
...
add_action('admin_bar_menu', 'billiardtoday_new_admin_bar_switcher', 999);

// Add theme switcher styles to admin
function billiardtoday_new_admin_styles() {
    echo '<style>
...
add_action('admin_head', 'billiardtoday_new_admin_styles');
add_action('wp_head', 'billiardtoday_new_admin_styles');

// Add theme switcher script
function billiardtoday_new_switcher_script() {
    if (!is_admin() && current_user_can('switch_themes')) {
        ?>
        <script>
        jQuery(document).ready(function($) {
            ...
            $('.theme-option').on('click', function(e) {
                ...
                $.ajax({
                    url: billiardtoday_new.ajax_url,
                    type: 'POST',
                    data: {
                        action: 'switch_theme',
                        theme: theme,
                        nonce: billiardtoday_new.nonce
                    },
...
add_action('wp_footer', 'billiardtoday_new_switcher_script');
?>
