 ...
 * @return array
 */
function billiardtoday_body_classes($classes) {
    ...
    $classes[] = 'billiardtoday-original';

    return $classes;
}
add_filter('body_class', 'billiardtoday_body_classes');

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function billiardtoday_pingback_header() {
    if (is_singular() && pings_open()) {
        printf('<link rel="pingback" href="%s">', esc_url(get_bloginfo('pingback_url')));
    }
}
add_action('wp_head', 'billiardtoday_pingback_header');

/**
 * Change the excerpt more string.
 */
function billiardtoday_excerpt_more($more) {
    return sprintf(
...
add_filter('excerpt_more', 'billiardtoday_excerpt_more');

/**
 * Custom comment callback.
 */
function billiardtoday_comment($comment, $args, $depth) {
    if ('div' === $args['style']) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 */
function billiardtoday_customizer_styles() {
...
add_action('wp_head', 'billiardtoday_customizer_styles');

/**
 * Add theme support for selective refresh for Customizer
 */
function billiardtoday_customize_partial_blogname() {
    bloginfo('name');
}
add_action('customize_partial_blogname', 'billiardtoday_customize_partial_blogname');

function billiardtoday_customize_partial_blogdescription() {
    bloginfo('description');
}
add_action('customize_partial_blogdescription', 'billiardtoday_customize_partial_blogdescription');

/**
 * Add WooCommerce support if WooCommerce is active
 */
if (class_exists('WooCommerce')) {
    function billiardtoday_woocommerce_setup() {
        add_theme_support('woocommerce');
        add_theme_support('wc-product-gallery-zoom');
        add_theme_support('wc-product-gallery-lightbox');
        add_theme_support('wc-product-gallery-slider');
    }
    add_action('after_setup_theme', 'billiardtoday_woocommerce_setup');
}

/**
 * Add custom page templates
 */
function billiardtoday_add_page_templates($templates) {
...
add_filter('theme_page_templates', 'billiardtoday_add_page_templates');

/**
 * Load custom page templates
 */
function billiardtoday_load_page_template($template) {
    global $post;
...
add_filter('template_include', 'billiardtoday_load_page_template');

/**
 * Add custom post types
 */
function billiardtheme_custom_post_types() {
...
add_action('init', 'billiardtheme_custom_post_types');

/**
 * Add custom taxonomies
 */
function billiardtheme_custom_taxonomies() {
...
add_action('init', 'billiardtheme_custom_taxonomies');

/**
 * Add meta boxes
 */
function billiardtheme_add_meta_boxes() {
    add_meta_box(
        'tournament_details',
        __('Tournament Details', 'billiardtoday'),
        'billiardtheme_tournament_details_callback',
        'tournaments',
        'normal',
        'high'
    );
}
add_action('add_meta_boxes', 'billiardtheme_add_meta_boxes');

function billiardtheme_tournament_details_callback($post) {
    wp_nonce_field('billiardtheme_save_tournament_details', 'tournament_details_nonce');
...
function billiardtheme_save_tournament_details($post_id) {
    if (!isset($_POST['tournament_details_nonce']) || !wp_verify_nonce($_POST['tournament_details_nonce'], 'billiardtheme_save_tournament_details')) {
        return;
    }
...
add_action('save_post', 'billiardtheme_save_tournament_details');
?>
