/**
 * Customizer JavaScript for BilliardToday Theme
 */
...
(function($) {
    ...
    wp.customize('accent_color', function(value) {
        value.bind(function(newval) {
            ...
            $('.feature-icon:hover, .stat-icon:hover').css({
                'background': 'linear-gradient(135deg, ' + newval + ' 0%, #00d9ff 100%)'
            });
        });
    });
    ...
    var style = $('<style>')
        .prop('type', 'text/css')
        .html('body, .btn-primary, .nav-cta, .feature-icon, .stat-icon { transition: all 0.3s ease; }')
        .appendTo('head');
