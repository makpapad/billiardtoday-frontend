<?php
/**
 * BilliardToday Sport Fun Theme Functions
 */
...
// Theme Setup
function billiardtoday_sport_fun_setup() {
    ...
    add_theme_support('custom-header', array(
        ...
        'flex-height'            => true,
        'flex-width'             => true,
        'wp-head-callback'       => 'billiardtoday_sport_fun_header_style',
    ));
}
add_action('after_setup_theme', 'billiardtoday_sport_fun_setup');

// Custom header style
function billiardtoday_sport_fun_header_style() {
...
function billiardtoday_sport_fun_scripts() {
    // Theme stylesheet
    wp_enqueue_style('billiardtoday-sport-fun-style', get_stylesheet_uri(), array(), '1.0.0');
    
    // Google Fonts
    wp_enqueue_style('billiardtoday-sport-fun-fonts', 'https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600;700;800;900&family=Fredoka+One&display=swap', array(), null);
    
    // Theme JavaScript
    wp_enqueue_script('billiardtoday-sport-fun-script', get_template_directory_uri() . '/js/theme.js', array('jquery'), '1.0.0', true);
    
    // Localize script
    wp_localize_script('billiardtoday-sport-fun-script', 'billiardtoday_sport_fun', array(
        'ajax_url' => admin_url('admin-ajax.php'),
        'nonce'    => wp_create_nonce('billiardtoday_sport_fun_nonce'),
    ));
}
add_action('wp_enqueue_scripts', 'billiardtoday_sport_fun_scripts');

// Register widget areas
function billiardtoday_sport_fun_widgets_init() {
...
add_action('widgets_init', 'billiardtoday_sport_fun_widgets_init');
...
function billiardtoday_sport_fun_switcher() {
    if (isset($_POST['theme']) && isset($_POST['nonce'])) {
        if (wp_verify_nonce($_POST['nonce'], 'billiardtoday_sport_fun_nonce')) {
...
add_action('admin_post_switch_theme', 'billiardtoday_sport_fun_switcher');

// AJAX Theme Switcher
function billiardtoday_sport_fun_ajax_switch_theme() {
    check_ajax_referer('billiardtoday_sport_fun_nonce', 'nonce');
...
add_action('admin_bar_menu', 'billiardtoday_sport_fun_admin_bar_switcher', 999);
...
function billiardtoday_sport_fun_admin_styles() {
    echo '<style>
        .theme-switcher-toggle { 
            ...
            box-shadow: 0 4px 6px rgba(255, 107, 53, 0.4) !important;
...
add_action('wp_head', 'billiardtoday_sport_fun_admin_styles');

// Add theme switcher script
function billiardtoday_sport_fun_switcher_script() {
    if (!is_admin() && current_user_can('switch_themes')) {
        ?>
        <script>
...
add_action('wp_footer', 'billiardtoday_sport_fun_switcher_script');
?>
